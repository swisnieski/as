...
namespace As
{
	// THREAD class reading IP scales  ===========================================
...
//			SCALOC *psScaloc = pSca->psScaloc;
	...
	//						 if(nCnt >= 16) 
							 {
								 ...
								 if(data[1] >= '0' && data[1] <= '9')  
									 fValue += (data[1]-'0')*100000;

								 if(data[2] >= '0' && data[2] <= '9')  
									 fValue += (data[2]-'0')*10000;

								 if(data[3] >= '0' && data[3] <= '9') 
									 fValue += (data[3]-'0')*1000;
	...
	//								 nValue = (int)(nValue * KG2LB);
						 ...
						 {
							 if(nCnt >= psIni->nSclLen)		//DC 3.2.5 SCALE_MSG)
							 {
								 nIdx = 0;
								 while((nIdx < nCnt) && (data[nIdx] != psIni->cSclFrame)) //DC 3.2.5 0xd))
									 nIdx++;

								 if((nCnt-nIdx) >= psIni->nSclLen)		//DC 3.2.5 SCALE_MSG)
								 {
									 nCnt = 0;
									 bSign = false;
									 if(nIdx == 0)
